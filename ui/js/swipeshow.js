...
define('swipeshow', ['jquery'], function ($) {
	...
	Swipeshow.prototype = {
		...
		init : function () {
			...
			//Set $holder width
			self.settings.$holder.css('width', (self.settings.imagesArray.length * 100) + '%');

			self.resetImages();
		},

		swipeshowWidth : function () {
			return this.settings.$element[0].scrollWidth;
		},

		imageWidth : function () {
			return Math.floor(this.settings.$element[0].scrollWidth / this.settings.imagesArray.length);
		},
		...
		resetImages : function (direction) {	
			...
			self.settings.$element.scrollLeft(self.imageWidth());
			self.settings.swipeshowLeftOffset = self.imageWidth();
